...
class FeatureGeneratorExplainer(Experiment):
    ...
    def __init__(self,  train_loader, valid_loader, test_loader, feature_size, historical=False, simulation=False,  experiment='feature_generator_explainer'):
        ...
        if simulation:
            ...
            self.risk_predictor_learned = EncoderRNN(feature_size,hidden_size=20,rnn='GRU',regres=True, return_all=True)
#x[0,2:50]*x[0,2:50] +  np.log(x[1,2:50]+3)*x[1,2:50] +  x[2,2:50]*x[2,2:50] -0.5
    ...
    def run(self, train):
        if train:
            self.train(self.feature_size, n_epochs=80)
        else:
            if os.path.exists('./ckpt/feature_0_generator.pt'):
                if not self.simulation:
                    ...
                    self.risk_predictor_learned.load_state_dict(torch.load('./ckpt/risk_predictor.pt'))
                    self.risk_predictor_learned = self.risk_predictor_learned.to(self.device)
                    self.risk_predictor_learned.eval()
            ...
            if not self.simulation:
                ...
                grad_out_true = np.array(grad_out)

                #In simulation data also get sensitivity w.r.t. a learned predictor
                self.risk_predictor_learned.train()
                signal = torch.Tensor(signal).to(self.device).requires_grad_()
                out = self.risk_predictor_learned(signal)
                grad_out_learned = []
            ...
            for _ in range(n_samples):
                ...
                if self.simulation:
                    if not learned_risk:
                        ...
                        predicted_risk = self.risk_predictor_learned(predicted_signal[:,0:t+1].view(1,predicted_signal.shape[0],t+1).to(self.device))[:,t].item()
                else:

...
cutoff = 7.9 # desired cutoff frequency of the filter, Hz
...
def main(n_samples, plot, Tt=48):
    ...
    y_train = np.array([butter_lowpass_filter(x[0,:], cutoff, fs, order) for x in x_train])
    #x_train_lpf = np.stack([x1_train_lpf, x2_train_lpf, x3_train_lpf],axis=1)

    y_test = np.array([butter_lowpass_filter(x[0,:], cutoff, fs, order) for x in x_test])
...
def generate_sample(plot, Tt=48):
    ...
    x1 = ts.signals.NARMA(order=2,coefficients=[.5,.5,1.5,.5],seed=1)
    x1_ts = ts.TimeSeries(x1, noise_generator=noise)
    x1_sample, signals, errors = x1_ts.sample(np.array(range(Tt)))

    
    seed = np.random.randint(1,1000)
    x2 = ts.signals.NARMA(order=2,coefficients=[.5,.5,1.5,.5],seed=1)
    noise = ts.noise.GaussianNoise(std=0.01)
    ...
    if trend==0 or trend==1:
        ...
        x2_sample[t[0]:t[1]] = x2_sample[t[0]:t[1]] + s*np.log(0.1*np.asarray(range(t[1]-t[0]))+1.)
        x2_sample[t[1]:t[2]] = x2_sample[t[1]:t[2]] + s * np.log(0.1 * np.full(x2_sample[t[1]:t[2]].shape, (t[1]-t[0])) + 1.)
        x2_sample[t[2]:t[3]] = x2_sample[t[2]:t[3]] - s*np.log(0.1*np.asarray(range(t[3]-t[2]))+1.)
    
    
    noise = ts.noise.GaussianNoise(std=0.01)
    x3 = ts.signals.NARMA(order=2,seed=100000)
    x3_ts = ts.TimeSeries(x3, noise_generator=noise)
    x3_sample, signals, errors = x3_ts.sample(np.array(range(Tt)))
    t = np.array(np.zeros(4))
    
    return np.stack([x2_sample, x3_sample]), t, trend_style[trend]
